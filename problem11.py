...
def problem11():
    ...
    for i in range(rows):
        for j in range(cols-N+1):
            M = max(M, product(grid[i][j:j+N]))

    # vertical
    for i in range(cols):
        for j in range(rows-N+1):
            M = max(M, product([col[i] for col in grid[j:j+N]]))

    # diagonals
    for i in range(rows-N+1):
        for j in range(cols-N+1):
            M = max(M, product([grid[i+k][j+k] for k in range(N)]))
            M = max(M, product([grid[i+k][N+j-k-1] for k in range(N)]))
